...
def run_inference(
    ...
    detections.add_attribute("sem_score", semantic_score)
    detections.add_attribute("appe_score", appe_scores)
    detections.add_attribute("geo_score", geometric_score)
    detections.add_attribute("scores", final_score)
    detections.add_attribute("scores_log", final_score_log)
    ...
    # Create Folder
    obj_output_dir = os.path.join(output_dir, "sam6d_results_01", obj_id)
    ...
    info_path = os.path.join(config["OUTPUT_DIR"], f"detection_{obj_id}_{image_id}.npz")
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(config["OUTPUT_DIR"], "sam6d_results_01"), exist_ok=True)
    ...
    for obj in config["OBJ_ID"]:
        ...
        num_image = 2 #len(os.listdir(obj_data_path)) - 2 
